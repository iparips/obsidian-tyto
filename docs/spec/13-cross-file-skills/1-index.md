# Cross-File Skills: Detailed Design

Implementable spec for release 7 of [2-plan.md](../../plan/plan.md). A delta on the
Desktop MVP; unlisted components are unchanged.

Discovery, frontmatter parsing and prompt injection ship in the MVPs. See
[1-desktop-mvp/3-component-design.md](../1-desktop-mvp/4-component-design.md).
This release lifts the single-note limit those releases declare.

- [2-requirements.md](2-requirements.md) - problem, goals, functional and non-functional requirements
- [3-component-design.md](3-component-design.md) - file tools, on-demand skill bodies, undo, trust boundary
