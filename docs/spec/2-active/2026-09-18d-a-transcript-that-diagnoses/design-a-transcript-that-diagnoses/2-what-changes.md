...
Three endings write nothing to the history at all. TurnOutcomes (Engine Turn Ending) builds Exhausted, Stuck and Failed, and only TurnEndingService (Engine) appends. A realistic exhausted turn therefore renders its last step's tool calls today, and the one path leaving an empty slice is a provider call that failed.
