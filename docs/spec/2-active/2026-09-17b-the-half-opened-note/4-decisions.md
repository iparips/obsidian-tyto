...
One change follows. The line names the cause today, and that cause is now one of
two:
...
neither. The design picks which, in the panel and in the transcript's
`written directly, undo not available`.
...
- The suite cannot catch this. FakeNoteLocator maps a path to an editor and
  FakeEditor holds its text from construction, so no test can build a view with
  the right path and the wrong contents. Stubbing such a check to return true
  left all 1319 tests green. The fakes need a half-opened view first.
