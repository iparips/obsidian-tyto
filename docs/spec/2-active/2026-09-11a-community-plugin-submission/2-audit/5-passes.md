...
No action. Worth listing, because it narrows the work: Tyto already avoids the
...
| No placeholder class names                  | TytoPlugin, TytoSettings, TytoSettingsTab                           |
...
| getAbstractFileByPath over iteration        | NoteReader resolves by path, without iterating                       |
...
| LICENSE present                             | LICENSE.md, AGPL-3.0-or-later; see finding 15 on its copyright line |
| README present and explains use             | README.md                                                           |
| minAppVersion set                           | 1.5.0; finding 14 raises it to 1.13.0                               |
...
| Trademark respected                         | Plugin is named Tyto, not an Obsidian-branded name                  |
...
- Action: rotate the key in the Mistral console, and keep the local file out of
  any archive or screenshot attached to the submission.
...
- Action: drop `--sourcemap` from `build` itself, not from a separate release
  script. Build verification runs `build`, per
  [6-the-new-process.md](6-the-new-process.md) finding 16. It also takes 1.3 MB
  of bundle down, since React is bundled unminified.
