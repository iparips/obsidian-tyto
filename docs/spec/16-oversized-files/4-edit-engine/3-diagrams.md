...
flowchart LR
    ...
    TurnConclusion --> NoteEditor

    classDef added fill:#4a5568,color:#fff
    class UtteranceQueue,ModelCaller,TurnConclusion added
```
...
the last edit when a turn succeeds, which is TurnConclusion's job after the
move. ChatProvider and HarnessTools go to ModelCaller, which is the only place
