...
Search-and-edit, creating a note at a model-chosen path, and multi-file writes. The first two are the same risk: a destination the model picked rather than one Obsidian computed. Multi-file writes arrive in [cross-file-skills/index.md](../spec/Upcoming/cross-file-skills/1-index.md).
