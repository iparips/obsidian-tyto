...
export class SkillSection {
  ...
  static rules(): string {
    return [
      ...
      'A tool whose schema declares applicable_skills is refused without it, so read the',
      'schema rather than guessing: the tools that declare it require it, and the rest',
      'neither need it nor take it. Name in it the skills covering the utterance.',
      'An empty list says none covers it. A skill you name',
      ...
      'Loading the skill is half of it. The refused call has not run, so make it again',
      'before anything else: a refused run_command opened no note, and the edit you make',
      'instead lands in whatever note the session was already on.',
      ...
      'The skill names the note it wants. Reach it with a listed command where one',
      'opens it, and search only where none does. A command opens the note on its',
      'own, where a path you found by searching is refused until the user picks it:',
      'searching for a note a command reaches asks them to confirm what they asked',
      'for.',
