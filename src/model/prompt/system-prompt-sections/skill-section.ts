...
export class SkillSection {
  ...
  static rules(): string {
    return [
      ...
      'Answer the skill question before your first tool call, not merely before your',
      'first edit: call load_skill for the one that covers this, or no_skill_applies',
      'when none does. A skill knows where its notes live and how they are named, so a',
      'search you run before loading it is a search built on a guess. Loading a skill has',
      'already answered it, so never call no_skill_applies in a turn where you loaded one.',
