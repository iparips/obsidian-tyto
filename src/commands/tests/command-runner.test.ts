...
describe('CommandRunner', () => {
  ...
  const runnerOf = (...entries: string[]) => {
    const app = appOf()
    return new CommandRunner(app, new CommandCatalogue(app, new AllowList(entries)))
  }

  describe('when the id is outside the catalogue', () => {
    it('returns a refusal naming the id when the id is not allowed', () => {
      const outcome = runnerOf('daily-notes:*').run('file-explorer:delete-file')
    ...
    it('never calls the registry when the id is not allowed', () => {
      runnerOf('daily-notes:*').run('file-explorer:delete-file')
  ...
  describe('when the command opens a note', () => {
    beforeEach(() => {
      registry.executeCommandById = (id: string) => {
        ...
        workspace.opens('Journal/2026-09-02.md')
        return true
      }
    })

    it('reports the opened path when the active note changes', () => {
      const outcome = runnerOf('daily-notes:*').run('daily-notes:goto-today')

      expect(outcome).toMatchObject({ value: { openedPath: 'Journal/2026-09-02.md' } })
    })

    it('rebinds when the active note changes', () => {
      const outcome = runnerOf('daily-notes:*').run('daily-notes:goto-today')

      expect(outcome.hasFailed() ? null : outcome.value.rebinds()).toBe(true)
    })

    it('names the new note in the tool result when the active note changes', () => {
      const outcome = runnerOf('daily-notes:*').run('daily-notes:goto-today')
  ...
  describe('when the command opens no note', () => {
    it('does not rebind when the active note is unchanged', () => {
      const outcome = runnerOf('daily-notes:*').run('daily-notes:goto-today')

      expect(outcome.hasFailed() ? null : outcome.value.rebinds()).toBe(false)
    })

    it('says the binding stayed when the active note is unchanged', () => {
      const outcome = runnerOf('daily-notes:*').run('daily-notes:goto-today')
    ...
    it('does not rebind when no note is active after the run', () => {
      registry.executeCommandById = (id: string) => {
        registry.executed.push(id)
