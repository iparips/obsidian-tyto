...
import { NoteOpenedByObsidianCommand } from './models/note-opened-by-obsidian-command'
...
export class ObsidianCommandRunner {
  constructor(
    private app: App,
  ...
  async run(commandId: string): Promise<Attempt<NoteOpenedByObsidianCommand>> {
    const allowed = this.commandCatalogue.resolve().find((command) => command.id === commandId)
  ...
  private unknownCommandMessage(commandId: string): string {
    const ids = this.commandCatalogue.resolve().map((command) => command.id)
  ...
  private async runAllowed(
    commandId: string,
    commandName: string,
  ): Promise<NoteOpenedByObsidianCommand> {
    const before = this.activePath()
    const opened = await this.openedNoteWait.forOpen(() =>
      this.registry.executeCommandById(commandId),
    )
    return ObsidianCommandRunner.noteOpenedBy(commandName, before, opened ?? this.activePath())
  }

  // Reopening the already-bound note counts as opening nothing, since the
  // binding does not move.
  private static noteOpenedBy(
    commandName: string,
    before: string | null,
    after: string | null,
  ): NoteOpenedByObsidianCommand {
    if (after === null || after === before)
      return NoteOpenedByObsidianCommand.openedNothing(commandName)
    return NoteOpenedByObsidianCommand.opened(commandName, after)
  }
